...
PhSpGeoDir = cd;
cd(KeplerDir)
ext = X{1,1}; stepS = X{1,2}; stepD = X{1,3}; RotS = X{1,4}; RotD = X{1,5};
...
sm_ax_o = 1;
f_o =   0;
S3prime_norm_init = InitData.init_S;
row_tot = InitData.row_tot_LSD;
D3prime_norm_init = InitData.init_D;
col_tot = InitData.col_tot_LSD;
phiS_o =  InitData.phiS_o;
phiD_o = -InitData.phiD_o;
E_tot = InitData.E_tot_LSD;
...
extstr = num2str(ext);
if row_tot <= col_tot
    D3prime_norm = D3prime_norm_init + stepD*(ext-1);
else
    S3prime_norm = S3prime_norm_init + stepS*(ext-1);
end
...
for int = 1:int_tot
    if row_tot <= col_tot
        S3prime_norm = S3prime_norm_init + stepS*(int-1);        
    else
        D3prime_norm = D3prime_norm_init + stepD*(int-1);
    end 

    S1 = sqrt(1 - S3prime_norm^2);
    S2 = 0;
    S3 = S3prime_norm;
...
%-------------------------------------------------------------------------
        ...
        try
            ...
            disp('the initial conditions for a correct value of S3 and D3.')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if InitData.Pert_LSD == 1  
            [Uo, Vo] = xy2uv(x_vec_o, y_vec_o);
        elseif InitData.NonPert_LSD == 1   
...
% do nothing
        ...
        if strcmp(get(InitData.warningbox, 'Checked'),'on')
...
%%%%%%%%%%%%%%%%%%%%%
    cd([KeplerDir, '/FMFT'])
        ...
        min_f = InitData.min_f1_LSD;
        max_f = InitData.max_f1_LSD;
    if (sign(min_f) ~= sign(max_f) | min_f == 0 | max_f == 0)
        ...
        min_f = InitData.min_f2_LSD;
        max_f = InitData.max_f2_LSD;
    if (sign(min_f) ~= sign(max_f) | min_f == 0 | max_f == 0)
        min_f = num2str(1000*min_f,'%6.12f');
        max_f = num2str(1000*max_f,'%6.12f');
        nfreq = num2str(2);
        ...
        min_f = InitData.min_f3_LSD;
        max_f = InitData.max_f3_LSD;
    if (sign(min_f) ~= sign(max_f) | min_f == 0 | max_f == 0)
        min_f = num2str(1000*min_f,'%6.12f');
        max_f = num2str(1000*max_f,'%6.12f');
        nfreq = num2str(2);
