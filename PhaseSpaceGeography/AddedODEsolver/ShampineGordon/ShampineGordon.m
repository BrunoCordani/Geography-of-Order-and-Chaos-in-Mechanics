...
function [tau_c,W] =  ShampineGordon(DiffEquation,time_span,y_init,options)
RelTol = options.RelTol;
AbsTol = options.AbsTol;
n_eqn = 8;
y = zeros(n_eqn, length(time_span));
y(:,1) = y_init;

for k = 1 : length(time_span) - 1;
    y(:,k + 1) = ShampineGordonStep(@DiffEquation,time_span(k),time_span(k+1), AbsTol, RelTol,n_eqn, y(:,k));
end
tau_c = time_span';
W = y';

function y = ShampineGordonStep(DiffEquation,t,tout,RelTol,AbsTol,n_eqn,y)
